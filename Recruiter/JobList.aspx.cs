...
namespace WebApplicationChanges.Recruiter
{
    public partial class JobList : System.Web.UI.Page
    {
        ...
        private void ShowJob()
        {
            ...
            query = @"SELECT Row_Number() over(Order by (Select 1)) as Sr_No, j.JobId, j.Title, j.NoOfPost, j.Qualification, j.Experience, j.LastDateToApply, j.CompanyName, j.CreateDate 
FROM jobs j
INNER JOIN recruiter r ON j.CompanyName = r.CompanyName
WHERE r.Username = @username;
";
            cmd = new MySqlCommand(query, con);
            cmd.Parameters.AddWithValue("@username", Session["recruiter"] ?? DBNull.Value);
            //cmd.Parameters.AddWithValue("@recruiterId", Session["recruiter"]);
