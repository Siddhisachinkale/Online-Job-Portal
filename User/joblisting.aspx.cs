    ...
    public partial class joblisting : System.Web.UI.Page
    {
        ...
        public static string RelativeDate(DateTime theDate)
        {
            ...
            foreach (long threshold in thresholds.Keys)
            {
                if (since < threshold)
                {
                   ...
                   (t.Days > 0 ? t.Days : (t.Hours > 0 ? t.Hours : (t.Minutes > 0 ? t.Minutes :
...

        {
...

            {
                ...
                string query = "SELECT JobId, Title, Salary, JobType, CompanyName, CompanyImage, CreateDate FROM Jobs \rWHERE DATE_FORMAT(CreateDate, '%Y/%m/%d') " + postedDate + " ";
                cmd = new MySqlCommand(query, con);
        ...
        string selectedRadioButton()
        {
            ...
            {
                postedDate = "=DATE('" + date.ToString("yyyy-MM-dd") + "')";
            }
            else if (RadioButtonList1.SelectedValue == "2")
            {
                postedDate = "BETWEEN DATE_SUB(DATE('" + date.ToString("yyyy-MM-dd") + "'), INTERVAL 2 DAY) AND DATE('" + date.ToString("yyyy-MM-dd") + "')";
            }
            else if (RadioButtonList1.SelectedValue == "3")
            {
                postedDate = "BETWEEN DATE_SUB(DATE('" + date.ToString("yyyy-MM-dd") + "'), INTERVAL 3 DAY) AND DATE('" + date.ToString("yyyy-MM-dd") + "')";
            }
            else if (RadioButtonList1.SelectedValue == "4")
            {
                postedDate = "BETWEEN DATE_SUB(DATE('" + date.ToString("yyyy-MM-dd") + "'), INTERVAL 5 DAY) AND DATE('" + date.ToString("yyyy-MM-dd") + "')";
            }
            else
            {
                postedDate = "BETWEEN DATE_SUB(DATE('" + date.ToString("yyyy-MM-dd") + "'), INTERVAL 10 DAY) AND DATE('" + date.ToString("yyyy-MM-dd") + "')";
            }
        ...
        protected void lbFilter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //                    queryList.Add(" Country ='" + ddlCountry.SelectedValue + isCondition = true; ");
                ...
                if (isCondition)
                {
                    ...
                    query = @"Select JobId, Title, Salary, JobType, CompanyName, Company Image, Country, State, CreatedDate from Jobs where " + subquery + " ";
                }
                else
                {
                    query = @"Select JobId, Title, Salary, JobType, CompanyName, Company Image, Country, State, CreatedDate from Jobs";
                }
